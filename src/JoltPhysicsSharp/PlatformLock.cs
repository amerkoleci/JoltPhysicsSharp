// https://github.com/mono/SkiaSharp/blob/main/binding/SkiaSharp/PlatformLock.cs
...
public static class PlatformLock
{
    ...
    partial class NonAlertableWin32Lock : IPlatformLock
    {
        public unsafe NonAlertableWin32Lock()
        {
            ...
            Kernel32.InitializeCriticalSectionEx(_cs, 4000, 0);
        }
        ...
        ~NonAlertableWin32Lock()
        {
            if (_cs != IntPtr.Zero)
            {
                Kernel32.DeleteCriticalSection(_cs);
        ...
        void Enter()
        {
            if (_cs != IntPtr.Zero)
            {
                Kernel32.EnterCriticalSection(_cs);
            }
        }
        ...
        void Leave()
        {
            if (_cs != IntPtr.Zero)
            {
                Kernel32.LeaveCriticalSection(_cs);
            }
        }
...
internal partial class Kernel32
{
    ...
    public static partial bool InitializeCriticalSectionEx(IntPtr lpCriticalSection, uint dwSpinCount, uint Flags);
    [LibraryImport("Kernel32.dll")]
    public static partial void DeleteCriticalSection(IntPtr lpCriticalSection);
    [LibraryImport("Kernel32.dll")]
    public static partial void EnterCriticalSection(IntPtr lpCriticalSection);
    [LibraryImport("Kernel32.dll")]
    public static partial void LeaveCriticalSection(IntPtr lpCriticalSection);
}
